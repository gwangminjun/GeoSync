# KRAS and GEOMEX-SYNC-HOME Structure
...
- `D:\IdeaProjects\local\기존 싱크\GEOMEX-SYNC-HOME`

분석 기준은 로컬 파일, JSP, `web.xml`, 설정 XML, 포함된 `.class` 문자열, `geomex-sync2.jar` 안의 `KrasWorker.java.bak`, 과거 로그다.

## 1. 핵심 결론

`GEOMEX-SYNC-HOME`의 KRAS 동기화는 `D:\IdeaProjects\local\기존 싱크\kras`의 `/conn/land_info` 같은 API를 직접 호출하지 않는다.

현재 설정상 `GEOMEX-SYNC-HOME`은 아래 URL을 직접 호출한다.
...
## 3. GEOMEX-SYNC-HOME 역할

`GEOMEX-SYNC-HOME`은 KRAS/KAIS 데이터를 수집해서 작업 파일과 PostgreSQL/PostGIS 대상 DB에 적재하는 동기화 배치/서비스 프로젝트다.
...
`geomex-sync2.jar` 안의 `KrasWorker.java.bak` 기준으로, KRAS 호출은 `HttpURLConnection`을 사용한 단순 POST다.
...
```xml
<servlet-class>geomex.kras.KrasConnCfg</servlet-class>
<load-on-startup>1</load-on-startup>

<servlet-class>geomex.koreps.KorepsConnCfg</servlet-class>
<load-on-startup>1</load-on-startup>

<servlet-name>KrasConnection</servlet-name>
<servlet-class>com.sun.jersey.spi.container.servlet.ServletContainer</servlet-class>
<param-value>geomex.kras.land;geomex.kras.bldg</param-value>
<url-pattern>/conn/*</url-pattern>

<servlet-name>GmxCtrl</servlet-name>
<param-value>geomex.kras.gmx.ctrl</param-value>
...
`geomex.kras.gmx.ctrl` 패키지 기반 GMX 조합 API다.
...
- 요청 자체를 GEOMEX-SYNC-HOME이 GPKI로 서명/암호화하는 구조는 확인되지 않았다.
...
1. `GEOMEX-SYNC-HOME` 실행 환경이 `http://110.20.1.12:8385/conn/estateGateway`에 접속 가능해야 한다.
...
| 의미 | 판단 |
|---|---|
| 로컬 `D:\IdeaProjects\local\기존 싱크\kras`를 켜야 한다 | 현재 설정상 GEOMEX-SYNC-HOME 직접 의존은 아님 |
| `110.20.1.12` 서버에 배포된 `kras` 또는 KRAS-KOREPSConn 서비스를 켜야 한다 | 맞을 가능성이 큼 |
| `estateGateway`를 제공하는 Tomcat/Java 서비스를 켜야 한다 | 맞음 |
| `kras` 테스트 UI를 켜야 GEOMEX-SYNC-HOME이 동작한다 | 현재 구조상 아님 |

## 7. 정상 동작 확인 방법

### 7.1 GEOMEX-SYNC-HOME 실행 서버에서 확인
...
하지만 이것은 `GEOMEX-SYNC-HOME`이 현재 호출하는 `estateGateway`와 다르다.
...
- GEOMEX-SYNC-HOME에서 복호화를 켰지만 인증서 설정이 없음
...
- GEOMEX-SYNC-HOME 자체 복호화 사용 여부 결정

### 8.3 kras API는 되는데 GEOMEX-SYNC-HOME은 실패

가능 원인:

- `kras` API는 토지/건축물 조회 API이고, GEOMEX-SYNC-HOME은 `estateGateway` 파일 다운로드 API를 사용한다.
- 두 기능은 호출 경로와 서비스 코드가 다르다.
- `kras`가 내부적으로 gateway를 호출할 때 성공하더라도 GEOMEX-SYNC-HOME 서버에서 gateway로 가는 네트워크가 막혀 있을 수 있다.
...
- `GEOMEX-SYNC-HOME`과 `kras`가 공통으로 바라보는 KRAS/KOREPS 연계 지점
