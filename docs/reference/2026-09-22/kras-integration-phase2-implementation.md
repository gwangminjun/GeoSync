...
| 항목 | 구현 내용 |
|---|---|
| 목록·상세 사이드바 | `/kras-db` 상단에 검색·상태 필터가 달린 연계 목록을 추가했다. 데이터셋 31개를 토지/건물/공간(SHP)/가격/전체TXT/기간조회 6개 그룹으로 분류하고, 클릭하면 기존 카드 위치로 이동해 2초간 테두리로 강조한다. |
| 카드 재사용 | 기존 카드 31개의 HTML·버튼·JS 함수는 수정하지 않았다. 각 카드에 `id="card-<slug>"`만 추가해 목록의 앵커 대상으로 삼았다. |
...
| Playwright / Edge | 기존 흐름(이력 선택/반영, 용도지역 순회, TXT 재수집 차단)이 그대로 동작함을 `kras-history.browser.cjs`로 재확인. 신규 `kras-nav.browser.cjs`로 검색·상태 필터·포커스 하이라이트·반영 전 비교 패널 렌더링을 검증 |
...
- 목록·상세는 "클릭 시 해당 카드로 이동 + 강조" 방식이다. 선택한 카드만 남기고 나머지를 숨기는 완전한 단일 패널 상세 화면은 아니다 — 31개 카드마다 카드 개수와 구성이 달라(일부 데이터셋은 카드 2~3개로 구성) 안전하게 감추고 복원하는 로직이 복잡해질 위험이 있어, 이번에는 이동+강조로 범위를 좁혔다.
